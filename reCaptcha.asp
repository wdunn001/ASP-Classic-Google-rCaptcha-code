Dim bCaptcha, sResponse, sCaptchaResponse, ErrorFlag, ErrorMsg
'**************
'init var
'**************
bCaptcha = True
ErrorFlag = False
'***********
'manipulate json as object class
'***********
Class aspJSON
	Public data
...
'***********************************************************************************
	ErrorFlag = True
	ErrorMsg = ErrorMsg & (ErrStr & "|")
End Sub

Function RequestReCaptcha(sReCaptcha)
'*********************************************************************************
' Filename:		RequestReCaptcha
' Project:
' Purpose:
' Notes:		
' History:		Developer		Description
' 04/08/2015	wjd				Initial Development
'*********************************************************************************
Dim sSecret, sResponse, sHost, sReturnStr, objHttpServer, sMethod, oJson
	sHost = "https://www.google.com/recaptcha/api/siteverify"
	oJson=_
         "secret=" & "SECRET_KEY_HERE" & _
         "&response=" & sReCaptcha
	'Response.Write oJson & "<br>"
	sMethod = "POST"
	sReturnStr = "" 
	Set objHttpServer = Server.CreateObject("Msxml2.ServerXMLHTTP")
	objHttpServer.setTimeouts 0,10000,20000,0	
	objHttpServer.open sMethod, sHost, false
	objHttpServer.setRequestHeader "Content-Type", "application/x-www-form-urlencoded"
	objHttpServer.Send oJson
	'Response.Write objHttpServer.Status & " : " & objHttpServer.StatusText & "<br>"
	sReturnStr = objHttpServer.responseText
	'Response.Write sReturnStr & "<br>"
	Set oJson = New aspJSON
	oJson.loadJson(sReturnStr)
	'Response.Write oJson
	Set objHttpServer = Nothing
	sResponse = oJson.data("success")
		'Response.Write sResponse & "<br>"
				If Not sResponse = "True" Then SetError("Captcha did not match")
End Function

If Len(Request.Form) > 0 Then
  If bCaptcha Then
  		 sCaptchaResponse = Request.Form("g-recaptcha-response")
  			If Len(sCaptchaResponse) > 0 Then
  			 RequestReCaptcha sCaptchaResponse
  			Else
  			 SetError("You must complete out the Captcha")
  			End If
  	End If
End If
<%If bCaptcha Then%>
...
<style>
.g-recaptcha {position:relative;left:50%%;margin-bottom:5px;}
</style>
<%End If%>
<%If bCaptcha Then%>
<div class="g-recaptcha" id="tbcaptcha" data-sitekey="YOUR_SITE_KEY"></div>
<%End If%>
